...
L.tileLayer("images/map/{z}/{x}/{y}.jpg",{minZoom:2,maxZoom:5,noWrap:!0,bounds:bounds}).addTo(map);if(-1!==window.location.search.search("debug"))L.marker([3E3,3E3],{draggable:!0}).bindPopup("").addTo(map).on("moveend",function(a){a=a.target;a.getPopup().setContent(a.getLatLng().toString());a.openPopup()});var overlay_materials={},key;
for(key in markers.materials){var material=markers.materials[key],layerGroup=L.layerGroup(),coords;for(coords in material.coords)coords=material.coords[coords],layerGroup.addLayer(L.marker(coords,{icon:L.icon({iconUrl:markers.icon_base+material.icon,iconSize:material.size})}));layerGroup.addTo(map);overlay_materials[key]=layerGroup}L.control.layers(null,overlay_materials).addTo(map);
map.on("click",function(a){isInBounds(a.latlng)&&L.popup().setLatLng(a.latlng).setContent("<p>Quadrant: "+getQuadrant(a.latlng)+"</p>").openOn(map)});function getQuadrant(a){var b=Math.floor((a.lat-395)/370);a=Math.floor((a.lng-16)/370)+1;b=Math.abs(b-19)+65;return String.fromCharCode(b)+" "+a}function isInBounds(a){return a.lat>bounds[0][0]&&a.lng>bounds[0][1]&&a.lat<bounds[1][0]&&a.lng<bounds[1][1]};
